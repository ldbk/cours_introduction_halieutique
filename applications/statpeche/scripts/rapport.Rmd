...
#effort
efforts<-fread("./applications/statpeche/data/efforts.csv")
#landings
landings1<-fread('./applications/statpeche/data/landings_2011_2023.csv')
landings2<-fread('./applications/statpeche/data/landings_2000_2010.csv')
...
lanmuryear<-lanMUR%>%
  group_by(year)%>%
  summarise(w=sum(landWt/1000,na.rm=T),price=mean(landValue,na.rm=T))

#plot
ggplot(lanmuryear,aes(x=year,y=w))+geom_point()+geom_smooth(method="lm")
#ggplot(lanmuryear,aes(x=year,y=price))+geom_point()+geom_smooth(method="lm")
