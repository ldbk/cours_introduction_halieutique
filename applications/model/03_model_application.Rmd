...
#verification
#on simule une dynamique de population avec les NQMF
simu1 <- discretelogistic(r=2.4,K=200,N0=100,Ct=25,Yrs=100)
plot(simu1)
#simu1 contient les informations
head(simu1)
#on modifie simu1 pour qu'il corresponde aux variables attendues
#data0<-data.frame(Year=simu1[,1],B=simu1[,2],Index=jitter(simu1[,2],amount=10)*0.4,Catch=rep(20,nrow(simu1)))
data0<-data.frame(Year=simu1[,1],B=simu1[,2],Index=simu1[,2]*1,Catch=rep(20,nrow(simu1)))
paraminit <- c(logr=log(0.5), logK=log(100), logBinit=log(80), logq=log(0.5))
Schaefer(paraminit,simu1bis)

est <- optim(paraminit, Schaefer, data=data0)
fit <- Schaefer(est$par, data0, verbose=TRUE)

exp(est$par)
fit$refpts

par(mfrow=c(2,2))
plot(data0$Year,fit$Ifit,main="Biomass Index",type="l")
points(data0$Year,data0$Index,pch="x",type="l",col="blue")
plot(data0$Year,fit$B,pch="x",type="l",main="Biomass estim")
plot(data0$Year,data0$B,pch="x",type="l",main="Biomass ori")
plot(data0$Year,fit$HR*fit$B,pch="x",type="l")
